/*****************************************************
     ...
     stdstrb.cpp - ANSI-C++ library: libio/iostream
 ----------------------------------------------------
...
#define STD_VTABLE (struct _IO_jump_t *)filebuf_vtable
#else
...
#else
#define DEF_STDFILE(NAME, FD, CHAIN, FLAGS) \
  struct _IO_FILE_plus NAME = {STD_VTABLE, FILEBUF_LITERAL(CHAIN, FLAGS, FD)}
#endif
...
struct _IO_fake_stdiobuf {    // behaves as streambuf
#ifdef __EK_BASED__
  struct {
#ifndef _VIRTUAL_IMPLEMENTATION  
    _IO_FILE file;
    ...
    void *far vtable;
#else
    const void *far vtable;
#endif
#else
//    const void *vt_stdiobuf;
//    const void *vt_filebuf;
#if defined(__USER_DEF_QUALIS__) && defined(__CONST_PTR_SIZE_2__)
    void *far vtable; // vt_streambuf
#else
    const void *far vtable; // vt_streambuf
#endif
    _IO_FILE file;
#endif
  } s;
...
//    const void *vt_filebuf;
#if defined(__USER_DEF_QUALIS__) && defined(__CONST_PTR_SIZE_2__)
    void *vtable; // vt_streambuf
#else
    const void *vtable; // vt_streambuf
#endif
    _IO_FILE file;
#endif
  } s;
...
extern struct _IO_jump_t stdiobuf_vtable;
#else
/* stdiobuf_vtable must be defined and initialized !! */
#undef stdiobuf_vtable
#define stdiobuf_vtable  _IO_streambuf_jumps
#endif
...
#ifdef __EK_BASED__
_IO_FILE *far _IO_list_all = &_IO_stderr_buf.s.file;
#else
