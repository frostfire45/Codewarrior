/*****************************************************
     ...
     genops.c - ANSI-C++ library: generic I/O operations
 ----------------------------------------------------
...
#include "comp.h"   /* HIWARE specific settings */
#include "libiop.h"
...
{
  register const char*_LIBDEF_FAR_CPP_PTR s = (char*) data;
...
DEFUN(_IO_adjust_column, (start, line, count),
      unsigned start AND const char*_LIBDEF_FAR_CPP_PTR line AND int count)
{
  register const char*_LIBDEF_FAR_CPP_PTR ptr = line + count;
  while (ptr > line)
    if (*--ptr == '\n')
