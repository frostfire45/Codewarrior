/*****************************************************
     ...
     stdstr.cpp - ANSI-C++ library: libio/iostream
 ----------------------------------------------------
...
#else
    _ios_fields *vb; /* pointer to virtual base class ios */
    void *vptr; /* pointer to virtual table of class ios */
#endif    
    _ios_fields base;
...
#define OSTREAM_DEF(NAME, SBUF, TIE, EXTRA_FLAGS) \
  _fake_ostream NAME = { {&NAME.base, 0}, {STD_STR(SBUF, TIE, EXTRA_FLAGS) }};
#define ISTREAM_DEF(NAME, SBUF, TIE, EXTRA_FLAGS) \
  _fake_istream NAME = {{&NAME.base, 0}, {STD_STR(SBUF, TIE, EXTRA_FLAGS)}};
#else
#define OSTREAM_DEF(NAME, SBUF, TIE, EXTRA_FLAGS) \
  _fake_ostream NAME = {STD_STR(SBUF, TIE, EXTRA_FLAGS)};
#define ISTREAM_DEF(NAME, SBUF, TIE, EXTRA_FLAGS) \
  _fake_istream NAME = {STD_STR(SBUF, TIE, EXTRA_FLAGS)};
#endif
#endif
...
#else
  extern ostream cout;
  _ios_fields _cout = {STD_STR(COUT_SBUF, NULL, 0) };
  _ios_fields _cerr = {STD_STR(CERR_SBUF, &cout, ios::unitbuf) };
  _ios_fields _cin  = {STD_STR(CIN_SBUF, &cout, 0)};
  _ios_fields _clog = {STD_STR(CERR_SBUF, &cout, 0)};
  ostream cout(&_cout);
  ostream cerr(&_cerr);
  istream cin(&_cin);
  ostream clog(&_clog);
