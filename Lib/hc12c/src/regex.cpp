/*****************************************************
     ...
     regex.cpp - ANSI-C++ library: Regex
 ----------------------------------------------------
...
Regex::Regex(const char*_LIBDEF_FAR_CPP_PTR t, int fast, int bufsize, 
               const char*_LIBDEF_FAR_CPP_PTR transtable)
{
...
#ifndef _NYI_LIB
  const char*_LIBDEF_FAR_CPP_PTR msg = re_compile_pattern((const char*_LIBDEF_FAR_CPP_PTR)t, tlen, buf);
  if (msg != 0)
...
int Regex::search(const char*_LIBDEF_FAR_CPP_PTR s, int len, int& matchlen, int startpos) const
{
...
int Regex::match(const char*_LIBDEF_FAR_CPP_PTR s, int len, int p) const
{
  if (p < 0)
  {
    p += len;
...
// can't verify much, since we've lost the original string
  ...
  if (!v) 
#ifndef _LIMITATION_HC05  
  (*lib_error_handler)("Regex", "invariant failure");
#else
  exit(0);
#endif
  return v;
