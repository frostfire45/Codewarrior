...
#ifndef _LIBRARY_WORKAROUND  /* non portable number (Assumes enum 32 bit) */
   re_floogle_flap = 65533
#else
   re_floogle_flap = 32767
#endif
};
...
inline int re_search_2 (struct re_pattern_buffer *rxb,
      __const__ char * string1, int size1,
      __const__ char * string2, int size2,
      int startpos, int range,
      struct re_registers *regs,
      int stop) {return 0;}
inline int re_search (struct re_pattern_buffer * rxb, __const__ char *string,
          int size, int startpos, int range,
          struct re_registers *regs) {return 0;}
inline int re_match_2 (struct re_pattern_buffer * rxb,
           __const__ char * string1, int size1,
           __const__ char * string2, int size2,
           int pos, struct re_registers *regs, int stop) {return 0;}
inline int re_match (struct re_pattern_buffer * rxb,
         __const__ char * string,
         int size, int pos,
         struct re_registers *regs) {return 0;}
inline reg_syntax_t re_set_syntax (reg_syntax_t syntax) {return syntax;}
inline void re_set_registers (struct re_pattern_buffer *bufp,
            struct re_registers *regs,
            unsigned num_regs,
            regoff_t * starts, regoff_t * ends) {}
inline __const__ char * re_compile_pattern (__const__ char *pattern,
          int length,
          struct re_pattern_buffer * rxb) {return pattern;}
inline int re_compile_fastmap (struct re_pattern_buffer * rxb) {return 0;}
inline char * re_comp (__const__ char *s) {return (char *)s;}
inline int re_exec (__const__ char *s) {return 0;}
inline int regcomp (regex_t * preg, __const__ char * pattern, int cflags) {return 0;}
inline int regexec (__const__ regex_t *preg, __const__ char *string,
        size_t nmatch, regmatch_t pmatch[],
        int eflags) {return 0;}
inline size_t regerror (int errcode, __const__ regex_t *preg,
      char *errbuf, size_t errbuf_size) {return errbuf_size;}
