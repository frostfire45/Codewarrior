/*****************************************************
     ...
     streamb.h - ANSI-C++ library: streambuf
 ----------------------------------------------------
...
{ // The data members of an ios.
    ...
    int _precision;
    void *far _arrays; /* Support for ios::iword and ios::pword. */
#else
    streambuf *_strbuf;
    ostream* _tie;
    int _width;
...
//    virtual int showmany ();
    virtual streamsize xsputn(const char*_LIBDEF_FAR_CPP_PTR s, streamsize n);
    virtual streamsize xsgetn(char* s, streamsize n);
    virtual streampos seekoff(streamoff, _seek_dir, int mode
    =ios::in|ios::out
    );
  ...
  if (base() || unbuffered()) return 0;
    ...
    streamsize sputn(const char*_LIBDEF_FAR_CPP_PTR s, streamsize n) {
  ...
  if (_IO_read_ptr >= _IO_read_end && __underflow(this) == EOF)
    ...
    virtual streamsize sys_write(const char*_LIBDEF_FAR_CPP_PTR, streamsize);
...
class filebuf : public streambuf {
  ...
  public:
    ...
    filebuf* open(const char*_LIBDEF_FAR_CPP_PTR filename, const char*_LIBDEF_FAR_CPP_PTR mode);
    filebuf* open(const char*_LIBDEF_FAR_CPP_PTR filename, ios::openmode mode, int prot = 0664);
    ...
    filebuf* close();
    virtual int doallocate();
    virtual streampos seekoff(streamoff so, _seek_dir, int mode
    =ios::in|ios::out
    );
    virtual streambuf* setbuf(char* p, int len);
    streamsize xsputn(const char*_LIBDEF_FAR_CPP_PTR s, streamsize n);
